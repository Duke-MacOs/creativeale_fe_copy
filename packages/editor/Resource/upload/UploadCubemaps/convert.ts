...
function copyPixelNearest(read: ImageData, write: ImageData) {
  ...
  const readIndex = (x: number, y: number): number => 4 * (y * width + x);
...
type F = (out: { x: number; y: number; z: number }, x: number, y: number) => void;
export interface IOrientations {
  pz: F;
  nz: F;
  px: F;
  nx: F;
...
const orientations: IOrientations = {
  pz: (out, x, y) => {
    out.x = -1;
    out.y = -x;
    out.z = -y;
...
interface IParams {
  data: ImageData;
  face: keyof IOrientations;
  rotation: number;
  interpolation: string;
  maxWidth?: number;
}
function generatorFace({
  data: readData,
  face,
  rotation,
  interpolation,
  maxWidth = 1024,
}: IParams): Promise<ImageData> {
  return new Promise((resolve, reject) => {
    try {
      const faceWidth = Math.min(maxWidth, readData.width / 4);
      const faceHeight = faceWidth;

      const cube: { x: any; y: number; z: number } = { x: 0, y: 0, z: 0 };
