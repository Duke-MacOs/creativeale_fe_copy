...
import { Space } from '../../TeamBoard/components';
import { useCurrentParams } from '../../TeamBoard';
      ...
      <Space size={1} style={{ flexGrow: 1, gridTemplateRows: 'auto auto 1fr' }}>
        ...
        >
          <Col className="gutter-row" span={8} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ color: '#999999', width: '90px', textAlign: 'end' }}>项目ID：</div>
            <Input
              allowClear
              style={{ width: '300px' }}
              value={search.id}
          ...
          {currentPage !== 'case' && (
            <Col className="gutter-row" span={8} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ color: '#999999', width: '90px', textAlign: 'end' }}>团队ID：</div>
              <Input
                allowClear
                style={{ width: '300px' }}
                value={search.teamId}
                onChange={e => {
                  onChangeSearch('teamId', e.target.value ? Number(e.target.value) : undefined);
                }}
              />
            </Col>
          )}
          ...
          {currentPage === 'case' && (
            <Col className="gutter-row" span={8} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ color: '#999999', width: '90px', textAlign: 'end' }}>playable_url：</div>
              <Input
                allowClear
                style={{ width: '300px' }}
                // value={search.playable_url}
                onChange={e => {
                  onChangeSearch('playable_url', e.target.value ?? undefined);
                }}
              />
            </Col>
          )}
